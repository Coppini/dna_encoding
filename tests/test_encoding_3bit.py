...
def test_encode_decode_known_examples(dna_sequence, expected_sequence):
    # Act
    encoded_dna = Encoded3bitSequence.from_sequence(dna_sequence)
...
def test_encode_invalid_bases(dna_sequence):
    ...
    with pytest.raises(ValueError):
        Encoded3bitSequence.from_sequence(dna_sequence)


def test_encode_quality_score_length_mismatch():
    # Act / Assert
    with pytest.raises(ValueError):
        Encoded3bitSequence.from_sequence("ATCG", "!#$%&'()*+,-./01234")
...
def test_encode_decode_random_fasta(header: bool):
    ...
    available_bases = sorted(ENCODING_TO_BASES[Encoding.BIT3_Ns_and_GAPs])
    ...
    # Act
    encoded_dna = Encoded3bitSequence.from_sequence(
...
def test_encoding_error_invalid_bases(sequence, invalid_bases):
    with pytest.raises(EncodingError) as exc_info:
        Encoded3bitSequence.from_sequence(sequence)
    assert exc_info.value.encoding == Encoding.BIT3_Ns_and_GAPs
