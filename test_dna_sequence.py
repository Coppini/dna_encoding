...
from dna_sequence import ENCODING_TO_BASES, DNASequence, EncodedSequence, Encoding
...
def test_encode_decode_known_examples(dna_sequence, expected_sequence, encoding_type):
    # Act
    encoded_dna = DNASequence.from_sequence(dna_sequence)
...
def test_encode_invalid_bases():
    ...
    with pytest.raises(ValueError):
        DNASequence.from_sequence("ATCGXYZ")


def test_encode_quality_score_length_mismatch():
    # Act / Assert
    with pytest.raises(ValueError):
        DNASequence.from_sequence("ATCG", "!#$%&'()*+,-./01234")


@pytest.mark.parametrize("encoding_type", list(Encoding))
def test_encode_decode_random_fasta(encoding_type):
    # Arrange
...
def test_encode_decode_random_fastq(
    encoding_type: Encoding, quality1: int, quality2: int, sort: bool
):
    # Arrange
    ...
    # Act
    encoded_dna = DNASequence.from_sequence(dna_sequence, quality)
